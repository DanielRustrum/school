    ...
    private static boolean fitClassRooms( int roomLevel )
       {
        ...
        boolean courseIsAvalible, courseFits;
        //Test if there is no more classes
        if(roomLevel > numRooms)
        {
            //Return true
            return true;
        }
        // Loop through classes per room
        for(index = 0; index < numRooms; index++) 
        {
            // Determine If Class fits
            courseIsAvalible = courseList[index].courseIsAvailable();
            courseFits = courseList[index].getSizeRequest() < roomList[roomLevel].getRoomCapacity();
            
            if(courseIsAvalible && courseFits)
            {
                // Make Course unavaliable
                courseList[index].setCourseAvailable(false);
                // If class can fit place class in room
                roomList[roomLevel].setAssociatedIndex(index);
                // Continue to next room
                fitClassRooms(roomLevel + 1);
            }
        }
        //Set Courses From Room To Be Availble
        for(index = numRooms;index >= roomLevel; index--) 
        {
            courseIndex = roomList[index].getAssociatedIndex();
            courseList[courseIndex].setCourseAvailable(true);
        }
        return false;
       }
            ...
            while( compareStrings( bldgName, "End_Classrooms" ) != 0 )
               {
                ...
                roomList[ roomIndex ] = new RoomClass( bldgName, roomNumber, roomCap );
            ...
            while( compareStrings( courseName, "End_Class_Requests" ) != 0 )
               {
                ...
                courseList[ courseIndex ] = new CourseClass( courseName, numStudents );
            ...
            while( index < maxLength && inCharInt != (int)( delimiterChar ) )
               {   
                if( inCharInt >= (int)( SPACE ) )
                   ...
                   else if( compareStrings( strBuffer, "Start Class Data;" ) == 0 )
                      {
